...
from os.path import exists,dirname,isfile, abspath
import socket
from shutil import copy as shutilCopy

dateAndTime = datetime.datetime.now().strftime('%Y_%m_%dT%H_%M_%S')
print(dateAndTime)
############## Public Variables Start #############
server = "localhost" #server address
port = 4001 #port of the server
mainTopic="trolly" # topic of the study. Everything that is published in this type is written and saved
studyName = "pilot" #Not Used yet # call it what the study is pilot, first or  second etc.
timeKEY='time' #
valueNameKEY = 'valueName'
valueKEY = 'value'
targetPath = getcwd() +'\\'+studyName+'\\'+dateAndTime+'\\'
#backupPath = getcwd() +'/backuptest/'+studyName+dateAndTime+'/'
backupPath = 'f:\\'+studyName+'\\'+dateAndTime+'\\'
...
print("Working directory: "+targetPath)
print("Backup directory: "+backupPath)
...
class uuidHandler:
	def __init__(self,uuid,keepalive):
		...
		self.file = open(targetPath+str(self.uuid)+".csv","w") # createa file based on the uuid
	...
	def close(self):  # called when the timer runs out
		...
		self.file.write("WARNING SCRAPDATA"+self.unparsedData)
		self.file.flush()  # flush the data
		self.file.close()  # the file is closed
		shutilCopy(abspath(self.file.name),backupPath+str(self.uuid)+".csv")
	def update(self):
		if(self.timer+self.keepalive<current_milli_time()): #checking if we have reached the maximum time
			print ("closing "+self.uuid+"with %i messages recieved" % self.counter) #reporting to the console that this class selfd istrcts
			self.close()
...
# the following function is there to calculate the running time of each uuid handler so they can figur out when to self distruct
...
thread.start_new_thread(runSocketServer, ())
...
count=0
while(1):
	print("---New Loop with  %i  active handler(s) and %i new messages---" %(int(len(handlerDict)),count) )
	count=0
	while(len(queue)>0):
		...
		try:
			...
			print(message)
			handlerDict[uuid].scrapData(message)
